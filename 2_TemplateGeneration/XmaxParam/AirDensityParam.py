...
DEFAULT_ATMOSPHERE_PATH = (
    Path(__file__).resolve().parent / "Data" / "Atmosphere.dat"
)


def rho_corsika(h_m, filename=None):
    """
    ...
    """
    if filename is None:
        filename = DEFAULT_ATMOSPHERE_PATH
...
if __name__ == "__main__":
    heights = np.linspace(3000, 15000, 1000)
    rho_values = np.array([rho_corsika(h) for h in heights])
